...
mod frontier_drop_only;
mod incident_reactive;
mod replay_reversal_gate;
mod rollback_budget;

use frontier_drop_only::FrontierDropOnlyVariant;
use incident_reactive::IncidentReactiveVariant;
use replay_reversal_gate::ReplayReversalGateVariant;
use rollback_budget::RollbackBudgetVariant;

const CASES_PATH: &str = "experiments/rollback_rules/cases.json";
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RollbackVariantCaseResult {
    pub case_id: String,
    pub correct: bool,
    pub expected_decision_kind: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RollbackSourceMetrics {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RollbackVariantReport {
    pub name: String,
    pub style: String,
    pub philosophy: String,
    pub source: RollbackSourceMetrics,
    pub cases: Vec<RollbackVariantCaseResult>,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RollbackExperimentReport {
    pub problem: String,
    pub generated_by: String,
    pub cases: Vec<RollbackCase>,
    pub variants: Vec<RollbackVariantReport>,
}
...
pub fn run_suite(root: &Path) -> Result<RollbackExperimentReport> {
    ...
    let variants: Vec<Box<dyn RollbackVariant>> = vec![
        Box::new(FrontierDropOnlyVariant::default()),
        Box::new(IncidentReactiveVariant::default()),
        Box::new(ReplayReversalGateVariant::default()),
        Box::new(RollbackBudgetVariant::default()),
    ];
    ...
    for variant in variants {
        ...
        for case in &cases {
            let decision = variant.decide(case).with_context(|| {
                format!(
                    "variant '{}' failed to decide rollback for case '{}'",
                    variant.name(),
                    case.id
                )
            })?;
            ...
            case_results.push(RollbackVariantCaseResult {
        ...
        reports.push(RollbackVariantReport {
    ...
    Ok(RollbackExperimentReport {
        problem: "Previously promoted runtime surface should be eligible for rollback through comparable rules instead of sticky one-way promotion.".to_string(),
...
pub fn render_summary(report: &RollbackExperimentReport) -> String {
...
pub fn render_experiments_section(report: &RollbackExperimentReport) -> String {
    let mut markdown = String::new();
    markdown.push_str("## Rollback Rules\n\n");
...
pub fn render_decisions_section(report: &RollbackExperimentReport) -> String {
    ...
    if let Some(reference) = provisional_frontier(report) {
        markdown.push_str(&format!(
            "- Provisional reference: `{}` because it stayed on the frontier while using the richest rollback evidence ({:.2} signals).\n",
            reference.name, reference.average_consumed_signals
        ));
    }
    markdown.push_str("- Keep rollback policy experimental until promoted surfaces actually survive and fail across multiple runtime cycles.\n");
    markdown
}
...
pub fn render_interfaces_section() -> String {
    ...
    markdown.push_str("pub struct RollbackCase {\n");
    markdown.push_str("    pub id: String,\n");
    markdown.push_str("    pub suite: String,\n");
    markdown.push_str("    pub promoted_reference: String,\n");
    markdown.push_str("    pub challenger_reference: String,\n");
    markdown.push_str("    pub current_frontier: Vec<String>,\n");
    markdown.push_str("    pub current_provisional_reference: String,\n");
    markdown.push_str("    pub replay_signal: String,\n");
    markdown.push_str("    pub contradiction_signal: String,\n");
    markdown.push_str("    pub regression_rounds: u32,\n");
    markdown.push_str("    pub support_gap: i32,\n");
    markdown.push_str("    pub environment_specific: bool,\n");
    markdown.push_str("    pub runtime_incident_severity: u32,\n");
    ...
    markdown.push_str("}\n\n");
    markdown.push_str("pub struct RollbackDecision {\n");
    ...
    markdown.push_str("}\n\n");
    markdown.push_str("pub trait RollbackVariant {\n");
    ...
    markdown.push_str("```\n\n");
    markdown
        .push_str("- Shared input: same promoted-surface drift tuple for every rollback policy.\n");
...
pub(crate) fn promoted_is_frontier(case: &RollbackCase) -> bool {
    case.current_frontier
        .iter()
        .any(|entry| entry == &case.promoted_reference)
}

pub(crate) fn challenger_is_frontier(case: &RollbackCase) -> bool {
    case.current_frontier
        .iter()
        .any(|entry| entry == &case.challenger_reference)
}

fn decision_matches(case: &RollbackCase, decision: &RollbackDecision) -> bool {
    decision.decision_kind == case.expected_decision_kind
        && decision.selected_reference == case.expected_reference
}

fn load_cases(root: &Path) -> Result<Vec<RollbackCase>> {
...
fn benchmark_variant(variant: &dyn RollbackVariant, case: &RollbackCase) -> Result<f64> {
...
fn collect_source_metrics(
    ...
    let hardcoded_decision_refs = decision_refs
        ...
        .sum();
    let evidence_refs = [
        "promoted_is_frontier(",
        "challenger_is_frontier(",
        "replay_signal",
...
fn readability_score(source: &RollbackSourceMetrics) -> u32 {
    clamp_score(
...
fn extensibility_score(source: &RollbackSourceMetrics) -> u32 {
    clamp_score(
...
fn frontier_variants(report: &RollbackExperimentReport) -> Vec<&RollbackVariantReport> {
...
fn provisional_frontier(report: &RollbackExperimentReport) -> Option<&RollbackVariantReport> {
